...
namespace Tech.Aerove.Lib.Public.WindowsNative
{
    /// <summary>
    /// Defines a set of utility methods for extracting icons for files and file 
    /// types.
    /// WAY BETTER alternative to Icon.ExtractAssociatedIcon which only supports 32x32
    /// </summary>
    public static class IconTools
    {

        /// <summary>
        /// Gets icon of a file and converts it to a png image uri
        /// </summary>
        /// <param name="path"></param>
        /// <returns></returns>
        public static string? GetIconAsImageUri(string path)
        {
            try
            {
                Icon icon = IconTools.GetIcon(path);
                using (MemoryStream ms = new MemoryStream())
                {
                    icon.ToBitmap().Save(ms, ImageFormat.Png);
                    string base64String = Convert.ToBase64String(ms.ToArray());
                    // var imageString = $"data:image/x-icon;base64,{base64String}";
                    var imageString = $"data:image/png;base64,{base64String}";
                    return imageString;
                }
            }catch(Exception e)
            {
        ...
        /// <summary>
        /// Gets the 256 x 256 icon for a file
        /// </summary>
        /// <param name="path"></param>
        /// <returns></returns>
